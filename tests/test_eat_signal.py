...
def test_probability_compose_signal_measures_probability_delta() -> None:
    model = build_eat_bart_model_from_config(
        BartConfig(
            d_model=16, encoder_layers=1, decoder_layers=1,
            encoder_attention_heads=2, decoder_attention_heads=2,
        ...
        EATAttentionConfig(2, 8, 8, 0.05, "probability_compose"),
        modify_decoder_self_attention=False,
    )
    batch = {
        "input_ids": torch.tensor([[0, 5, 6, 2]]),
        "attention_mask": torch.ones(1, 4, dtype=torch.long),
        "encoder_emotion_features": torch.randn(1, 4, 8),
    }
    rows, summary = calculate_encoder_eat_signal(model, [batch])
    assert all(row["signal_space"] == "probability_delta" for row in rows)
    assert summary["overall_r_h"] > 0
